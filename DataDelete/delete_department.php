...
// Check if the request method is POST and if departmentid is set
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['departmentid'])) {
    $departmentid = $_POST['departmentid'];

    // Fetch category name before deletion for logging purposes
    $queryFetchName = "SELECT Title FROM departments WHERE DepartmentID = ?";
    $stmtFetchDepartmentName = $conn->prepare($queryFetchName);
    $stmtFetchDepartmentName->bind_param("i", $departmentid);
    ...
    if ($resultFetchDepartmentName && $resultFetchDepartmentName->num_rows > 0) {
        ...
        if (isset($_SESSION['Username'])) {
            ...
            if ($resultUserCheck && $resultUserCheck->num_rows > 0) {
                ...
                $action = 'DELETE';
                $activity = 'Delete department title: ' . $departmentName;
        ...
        $query = "UPDATE departments SET Active = 0 WHERE DepartmentID = ?";
